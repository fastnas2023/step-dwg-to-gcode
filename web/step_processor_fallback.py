    ...
    def process(self, preview_only=False, preview_output=None):
        ...
        if preview_only:
            if not preview_output:
                # 生成默认预览输出路径
                preview_dir = os.path.join("web", "static", "plots", str(uuid.uuid4()))
                os.makedirs(preview_dir, exist_ok=True)
                preview_output = os.path.join(preview_dir, "step_preview.png")
        ...
        # 计算统计信息
        stats = {
            "points": len(self.points_array) if self.points_array is not None else 0,
            "edges": len(self.edges_array) if self.edges_array is not None else 0,
            "bounds": self.bounds if self.bounds else (0, 0, 0, 0, 0, 0)
        }
        
        # 对于完整处理，我们只返回基本信息
        return self.edges_array, self.bounds, stats 