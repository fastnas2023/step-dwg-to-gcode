...
def parse_arguments():
    ...
    parser.add_argument('-f', '--feed_rate', type=float, default=500.0, help='进给速率 (mm/min)')
    parser.add_argument('-s', '--safety_height', type=float, default=10.0, help='安全高度 (mm)')
    parser.add_argument('-d', '--cut_depth', type=float, default=0.5, help='切削深度 (mm)')
    parser.add_argument('-v', '--visualize', action='store_true', help='生成可视化图形')
    parser.add_argument('-c', '--controller', type=str, default='fanuc', help='CNC控制器类型: fanuc, siemens, heidenhain, haas')
...
def main():
    ...
    try:
        ...
        if output_file is None:
            output_file = os.path.splitext(input_file)[0] + f"_{controller_type}_gcode.nc"
        ...
        print(f"提取了 {len(edges)} 条边")
        
        # 尝试导入CNC控制器工厂
        try:
            from cnc_controller_factory import CncControllerFactory
            # 使用工厂创建控制器
            gcode_generator = CncControllerFactory.create_controller(
                controller_type,
                feed_rate=feed_rate,
                safety_height=safety_height,
                cut_depth=cut_depth
            )
            print(f"使用 {controller_type.upper()} 控制器生成G代码")
        except (ImportError, ValueError):
            # 如果导入失败或控制器类型无效，使用默认FANUC生成器
            print("使用默认FANUC G代码生成器")
            gcode_generator = NumPyFanucGcodeGenerator(
                feed_rate=feed_rate,
                safety_height=safety_height,
                cut_depth=cut_depth
            )
        ...
        # 生成G代码
        gcode = gcode_generator.generate_gcode(edges)
