...
"""
STP文件转FANUC G代码转换器 (无NumPy依赖版本)
此脚本读取STEP格式的CAD文件并生成适合FANUC系统的G代码，不依赖NumPy库
"""
...
class FanucStepToGcodeNoNumpy:
    ...
    def parse_step_file(self):
        ...
        if self.vertices:
            ...
            print(f"模型边界: X: {min_x:.3f} 到 {max_x:.3f}, Y: {min_y:.3f} 到 {max_y:.3f}, Z: {min_z:.3f} 到 {max_z:.3f}")
...
def main():
    parser = argparse.ArgumentParser(description='将STEP文件转换为FANUC G代码 (无NumPy依赖版本)')
    ...
    converter = FanucStepToGcodeNoNumpy(
